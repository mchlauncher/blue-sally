  ...
  authKeys.forEach(val => {
    ...
    const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="https://cravatar.eu/helmhead/${
      acc.uuid
    }">
            </div>
          ...
          submodules ? 'Sub' : ''
                    ...
                    ${
                      mdl.subModules.length > 0
                        ...
                        ${Object.values(
                          parseModulesForUI(mdl.subModules, true, servConf[mdl.getVersionlessMavenIdentifier()])
                        ).join('')}
                    </div>`
                        : ''
                    }
                            ...
                            <div class="settingsModDetails">
                                ...
                                <div class="settingsDropinRemoveWrapper">
                                    <button class="settingsDropinRemoveButton" remmod="${
                                      dropin.fullName
                                    }">${Lang.queryJS('settings.dropinMods.removeButton')}</button>
                                </div>
                            </div>
                        </div>
                        <label class="toggleSwitch">
                            <input type="checkbox" formod="${dropin.fullName}" dropin ${
      !dropin.disabled ? 'checked' : ''
    }>
                            <span class="toggleSwitchSlider"></span>
                        </label>
                    </div>
...
function populateReleaseNotes() {
  $.ajax({
    url: 'https://github.com/mchlauncher/blue-sally/releases.atom',
    success: data => {
